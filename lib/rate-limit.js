...
/**
 * Which backend is answering, for the response header.
 *
 * "memory" is the honest signal that limits are per-instance and therefore
 * leaky, so it is worth being able to see it from outside rather than having
 * to guess whether the Redis credentials actually arrived.
 */
export const rateLimitStore = () => (usingRedis ? 'redis' : 'memory')
